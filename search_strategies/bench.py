...
def process_results(epsilons, results, save_path, metric_name):
    ...
    for epsilon in epsilons:
        ...
        row['accuracy'] = np.mean(accuracy_train)
        row['unfairness'] = np.mean(unfairness_train)
        row['accuracy_test'] = np.mean(accuracy_test)
        row['unfairness_test'] = np.mean(unfairness_test)
