...
# Train it
clf.fit(X_train_unprotected, y_train, features=features[2:], prediction_name="(credit_rating:yes)", time_limit = max_time, memory_limit=max_memory)# max_evals=100000) # time_limit=8100, 
...
if rank == 0:
    if max_time is None: 
        fileName = './results_learning_quality_german_credit/%s_eps%f_metric%d_LB%d_%s_broadwell.csv' %(dataset, epsilon, fairnessMetric, filteringMode, policy)
    else:
        fileName = './results_learning_quality_german_credit/%s_eps%f_metric%d_LB%d_%s_tLimit%d_broadwell.csv' %(dataset, epsilon, fairnessMetric, filteringMode, policy, max_time)
