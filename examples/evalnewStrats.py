...
strategies = ["bfs-objUnf","best-ObjUnf","best-BFSObjUnf"]
strategy = "bfs"
if args.strat == 1:
	bfsmode = 5 
elif args.strat == 2:
...
def run():
    if forbidSensAttr:
        ...
        suffix = "sens-arg"
    filename = "./results/strat-%s-%s-%s.csv" %(strategy, suffix, metricName)
    if bfsmode == 2:
        filename = "./results/strat-bfs-obj-%s-%s.csv" %(suffix, metricName)
    rowlist = []
