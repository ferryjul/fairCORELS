...
def performKFoldSepTest(foldID, lambdaF, minFairness=0.0, modeC=3):
    ...
    cmTest = ConfusionMatrix(X_test[:,ProtectedIndex], X_test[:,UnprotectedIndex], y_pred_test, y_test)
...
X_test, y_test, _, _ = load_from_csv("data/adult_test_binary.csv")
print("--- DATASET INFO --- ")
...
returnList = Parallel(n_jobs=-1)(delayed(performKFoldSepTest)(foldID=_foldID, modeC=2, lambdaF=0.001) for _foldID in range(kFold))
...
returnList = Parallel(n_jobs=-1)(delayed(performKFoldSepTest)(foldID=_foldID, minFairness=0.0, lambdaF=bestLambda) for _foldID in range(kFold))
accuracy_list_test = []
statistical_parity_list_test = []
predictive_parity_list_test = []
predictive_equality_list_test = []
...
while unfairnessLim < fairness_list_train_tot[1][fairnessMetric-1]:
    ...
    bestLambda = runHyperOpt().get('lambdaFactor')
    returnList = Parallel(n_jobs=-1)(delayed(performKFoldSepTest)(foldID=_foldID, minFairness=1-unfairnessLim, lambdaF=bestLambda) for _foldID in range(kFold))
    accuracy_list_test = []
    statistical_parity_list_test = []
    predictive_parity_list_test = []
    predictive_equality_list_test = []
...
plt.savefig("./plots/result_plot_fairness_%d_val.png" %fairnessMetric)

with open('./plots/results_fairness_%d_val.csv' %fairnessMetric, mode='w') as csv_file:
