...
import argparse
from faircorels import load_from_csv, CorelsBagging, ConfusionMatrix, Metric
import csv
import time
...
baggingRatio = 1.0 # Size of the bootstrap sampling subsamples
...
def oneFold(foldIndex, X_fold_data): # This part could be multithreaded for better performance
    ...
    sensVect =  X_train[:,sensitive_attr_column] # 32 = female 
    unSensVect =  X_train[:,unsensitive_attr_column] # 33 = male (note that here we could directly give the column number)
    ...
    # Create the CorelsBagging object
    clf = CorelsBagging(X_train[:,2:], 
                        y_train,
                        n_learners=n_learners,
                        sample_size=int(X_train.shape[0]*baggingRatio), 
                        features=features[2:], 
    ...
    print("Fold ", foldIndex, " training accuracy ", accTraining)
    print("Fold ", foldIndex, " training unfairness ", unfTraining)
    objF = ((1-accTraining) + (lambdaParam*length))
 
    return [foldIndex, accTraining, unfTraining, objF, acc, unf, np.average(clf.nbExploredList), np.average(clf.nbCacheList), length]

# Multithreading is included in the CorelsBagging object (given parameter n_workers)
...
# save results (running statistics are averaged among all the base learners)
