...
if max_time is None: 
    fileName = './results/%s_eps%f_metric%d_LB%d_%s_single.csv' %(dataset, epsilon, fairnessMetric, filteringMode, policy)
else:
    fileName = './results/%s_eps%f_metric%d_LB%d_%s_tLimit%d_single.csv' %(dataset, epsilon, fairnessMetric, filteringMode, policy, max_time)
