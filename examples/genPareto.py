...
def performKFold(foldID, minFairness=0.0, modeC=3):
    ...
    c = CorelsClassifier(verbosity=[],map_type="prefix", n_iter=NBNodes, c=0.0005, max_card=1, policy="bfs", bfs_mode=2, useUnfairnessLB=True, fairness=fairnessMetric, maj_pos=UnprotectedIndex+1, min_pos=ProtectedIndex+1, epsilon=minFairness, mode=modeC)
...
delta = dist/NBPoints
fairnessLim = fairness_list_test_tot[0][fairnessMetric-1] + delta
while fairnessLim < fairness_list_test_tot[1][fairnessMetric-1]:
    ...
    predictive_equality_list_train = []
    equal_opportunity_list_train = []
    print("initial solution for max accuracy generated")
    for aReturn in returnList:
        accuracy_list_train.append(aReturn[0])
        accuracy_list_test.append(aReturn[1])
        statistical_parity_list_train.append(aReturn[2])
